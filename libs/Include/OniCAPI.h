...
ONI_C_API OniStatus oniInitialize(int apiVersion, const char* appLibPath = NULL);
/**  Shutdown OpenNI2 */
...
/** Return depth value unit in mm from format. Return 1.0 if it is not a valid depth format **/
ONI_C_API float oniGetDepthValueUnit_mm(const OniPixelFormat format);
